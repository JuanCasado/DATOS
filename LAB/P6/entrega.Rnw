...
plot(data$Poblacion, data$Asientos, xlab='Poblacion', ylab='Tamaño del parlamento (en escaños)')
abline(regresion)
@
\end{center}
...
legend("topleft", legend=c("Regresión", "Regresion+-desviación típica"),
      col=c("black", "blue"), lty=c(1,2), lwd=c(3,2))
@
\end{center}
...
\begin{center}
<<ggplot2_NO_se, fig=TRUE, echo=FALSE>>=
ggplot(data = datos3, aes(x = datos3$Temperature_ElMonte, y = datos3$Temperature_Sandburg)) + 
